...
def add_tables(cnx, cursor, db_name, isDetailed):
    cnx.database = db_name
    if isDetailed:
        TABLES[var.TABLE_NAME_COMP] = var.LARGE_TABLE['create_main_table']
    else:
        TABLES[var.TABLE_NAME_SIMP] = var.SIMPLE_TABLE['create_main_table']
...
def add_game_simp(cnx, cursor, info):
    cursor.execute(var.SIMPLE_TABLE['insert_game'], info)
    cnx.commit()


def get_games_simp(cnx, cursor):
    cnx.database = var.DB_NAME
    cursor.execute(var.SIMPLE_TABLE['select_game'])
    results = cursor.fetchall()
    return results
